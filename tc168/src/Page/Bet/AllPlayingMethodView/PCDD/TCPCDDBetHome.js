...
export default class TCMarkSixBetHome extends React.Component {
        ...
        return (
            <View style={styles.container}>
                ...
                <AwardCoundtdownView
                    ...
                    timeOutCallBack = {()=>{this.getPlanNoDetailRequest(true)}}
                />
    ...
    componentDidMount() {
        ...
        this.listener = RCTDeviceEventEmitter.addListener('TCMarkSixSelectView_numberSelected', (areaIndex, number, isAdd) => {
            ...
            if (SingletonDPS.getmyMathTypeID() == 'SP3'&&isAdd){
                let array = SingletonDPS.getAreaArrayWith(0)
                if (array.length>3){
                    SingletonDPS.removeNumberWithType(0,array[0]==number?array[1]:array[0])
                    this.resetSelectedNumber(SingletonDPS.getWillAddNumbersDic())
                }
            }
    ...
    //初始化玩法号码选择
    initialContentView() {
        return <MarkSixMainView ref='MarkSixMainView' gameUniqueId={this.props.gameUniqueId} shakeEvent={()=>this.byShake()}/>
    }
