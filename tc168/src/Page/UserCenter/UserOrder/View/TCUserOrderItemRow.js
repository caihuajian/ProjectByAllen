            ...
            <View>
                <View style={styles.itemStyle}>
                    <View style={styles.itemLeftStyle}>
                        ...
                        <Text style={styles.itemContent}>共{orderData.totalUnits}注</Text>
                    </View>

                    <View style={styles.itemMidStyle}>
                        <Text style={styles.itemTitle}>第{orderData.gameIssueNo}期</Text>
                        <Text style={styles.itemContent}>{orderData.bettingTime}</Text>
                    </View>
                    <View style={styles.itemRightStyle}>
                        <Text style={styles.itemContent1}>{(orderData.transactionAmount).toFixed(2)} 元</Text>
    ...
    getOrderState(orderData) {
        switch (orderData.transactionState) {
            case 'PENDING':
                return '待开奖'
            case 'WIN':
                return (<Text style={{color:Color.text.red1}}>中{(orderData.winAmount).toFixed(2)}元</Text>)
            case 'LOSS':
                return '未中奖'
        }
    }
