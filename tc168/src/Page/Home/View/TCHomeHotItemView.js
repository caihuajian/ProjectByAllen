...
export default class TCHomeHotItemView extends React.Component {
        ...
        return (
            <TouchableOpacity style={styles.container} onPress={()=> {
                ...
                <View style={{marginLeft: 10, justifyContent: 'center', width: width / 2 - 80} }>
                    <Text style={{color: colorArray[index], fontSize: 16}} ellipsizeMode='tail' numberOfLines={1}> {this.props.rowData.gameNameInChinese} </Text>
                    <Text style={{color: '#999999', fontSize:width>=375?14:12, marginTop: 8}} ellipsizeMode='tail' numberOfLines={1}> {this.props.rowData.gameDescription} </Text>
                </View>
                {this.getImage()}
            </TouchableOpacity>
        )
    ...
    getImage(){
        ...
        if (this.props.rowData&&this.props.rowData.status&&this.props.rowData.status == 'FORBIDDEN'){
            <Image source={{uri:this.props.rowData.status=='FORBIDDEN'?this.props.rowData.gameIconGrayUrl:this.props.rowData.gameIconUrl}} style={styles.leftImgStyle}/>
        }else {
